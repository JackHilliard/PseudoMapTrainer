...
class MapTRv2Head(DETRHead):
    ...
    def loss_single(self,
        ...
        """
        num_imgs = cls_scores.size(0) # num_imgs is obsolete naming of DETR, this is actually the batch size
        bs = num_imgs
        ...
        normalized_pts_targets = normalize_2d_pts(pts_targets, self.pc_range) if not self.z_cfg['gt_z_flag'] else normalize_3d_pts(pts_targets, self.pc_range)
        ...
        valid_mask = torch.isfinite(pts_preds.reshape(pts_preds.shape[0], -1)).all(dim=-1)

        if self.filter_invalid_pred:
            # NOTE: Differentiable rasterization is a bit slow, so we need to first filter out all invalid predictions
            #      (those predictions with no matched ground truth) and only rasterize the valid ones.
        ...
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            ...
            loss_rendered_mask = torch.nan_to_num(loss_rendered_mask)
            loss_dir = torch.nan_to_num(loss_dir)

        return loss_cls, loss_bbox, loss_iou, loss_pts, loss_rendered_mask, loss_dir
